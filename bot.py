    ...
    if pending:
        ...
        if duration is not None:
            ...
            if pending.get('answered'):
                # ÐÐ´Ð¼Ð¸Ð½ Ó©Ð¼Ð½Ó© Ð½ÑŒ Ñ…Ð°Ñ€Ð¸ÑƒÐ»ÑÐ°Ð½ Ñ‡ Ð±ÑƒÑ€ÑƒÑƒ Ð±Ð¸Ñ‡ÑÑÐ½ Ð³ÑÐ¶ Ð´Ð°Ñ…Ð¸Ð½ Ð¸Ð»Ð³ÑÑÐ¶ Ð±Ð°Ð¹Ð½Ð° â€”
                # Ñ…ÑƒÐ³Ð°Ñ†Ð°Ð°Ð³ ÐžÐ”ÐžÐžÐ“ÐžÐžÐ¡ ÑˆÐ¸Ð½ÑÑÑ€ Ñ‚Ð¾Ð³Ñ‚Ð¾Ð¾Ð½Ð¾ (Ð½ÑÐ¼ÑÑ…Ð³Ò¯Ð¹, Ð¾Ñ€Ð»ÑƒÑƒÐ»Ð½Ð°).
                expiry = db.set_vip(target_user_id, chat_id, duration, chat_title=chat_title)
                expiry_str = expiry.strftime('%Y-%m-%d %H:%M')
                await message.reply_text(
                    f"ðŸ”„ Ð¥ÑƒÐ³Ð°Ñ†Ð°Ð° Ð·Ð°ÑÐ²Ð°Ñ€Ð»Ð°Ð³Ð´Ð»Ð°Ð°\n"
                    f"ðŸ‘¤ {username}\n"
                    f"ðŸ†” {target_user_id}\n"
                    f"ðŸ“º {chat_title or chat_id}\n"
                    f"ðŸ“… Ð¨Ð¸Ð½Ñ Ð´ÑƒÑƒÑÐ°Ñ…: {expiry_str}"
                )
                try:
                    await context.bot.send_message(
                        chat_id=target_user_id,
                        text=f"â„¹ï¸ Ð¢Ð°Ð½Ñ‹ VIP Ð´ÑƒÑƒÑÐ°Ñ… Ñ…ÑƒÐ³Ð°Ñ†Ð°Ð° ÑˆÐ¸Ð½ÑÑ‡Ð»ÑÐ³Ð´Ð»ÑÑ.\nðŸ“º {chat_title or chat_id}\nðŸ“… Ð¨Ð¸Ð½Ñ Ð´ÑƒÑƒÑÐ°Ñ… Ð¾Ð³Ð½Ð¾Ð¾: {expiry_str}"
                    )
                ...
                await message.reply_text(
                    ...
                    f"âš ï¸ Ð¥ÑƒÐ³Ð°Ñ†Ð°Ð° Ð±ÑƒÑ€ÑƒÑƒ Ð±Ð¾Ð» Ð´Ð°Ñ…Ð¸Ð½ Ð·Ó©Ð² Ñ…ÑƒÐ³Ð°Ñ†Ð°Ð° Ð±Ð¸Ñ‡Ð¸Ñ…ÑÐ´ ÑˆÑƒÑƒÐ´ Ð·Ð°ÑÐ½Ð°."
                )
                try:
                    await context.bot.send_message(
                        chat_id=target_user_id,
            ...
            return
        # Ð¤Ð¾Ñ€Ð¼Ð°Ñ‚ Ñ‚Ð°Ð°Ñ€Ð°Ñ…Ð³Ò¯Ð¹ Ð±Ð¾Ð» Ð´Ð¾Ð¾Ñˆ Ð½ÑŒ Ò¯Ñ€Ð³ÑÐ»Ð¶Ð¸Ð»Ð½Ñ (Ó©Ó©Ñ€ ÐºÐ¾Ð¼Ð°Ð½Ð´ Ð³ÑÐ¶ Ò¯Ð·Ð½Ñ)

    if text == '/done' and pending:
        context.bot_data.pop('pending_vip', None)
        await message.reply_text("âœ… Ð”ÑƒÑƒÑÐ»Ð°Ð°. Ð¥ÑƒÐ³Ð°Ñ†Ð°Ð° Ð·Ð°ÑÐ°Ñ… Ð³Ð¾Ñ€Ð¸Ð¼ Ñ…Ð°Ð°Ð³Ð´Ð»Ð°Ð°.")
        return
...
async def handle_chat_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if old_status in ['left', 'kicked'] and new_status in ['member', 'administrator']:
        # Ð¥ÑÑ€ÑÐ² ÑÐ½Ñ Ñ…ÑÑ€ÑÐ³Ð»ÑÐ³Ñ‡Ð¸Ð¹Ð³ ÑÐ°Ñ handle_join_request_approved (channel-Ð¸Ð¹Ð½
        # "Approve new members" Ð³Ð¾Ñ€Ð¸Ð¼) Ð°Ð»ÑŒ Ñ…ÑÐ´Ð¸Ð¹Ð½ Ð°ÑÑƒÑƒÑÐ°Ð½ Ð±Ð¾Ð» Ð´Ð°Ð²Ñ…Ð°Ñ€Ð´ÑƒÑƒÐ»Ð¶
        # Ð´Ð°Ñ…Ð¸Ð½ Ð±Ð¸Ñ‡Ð¸Ñ…Ð³Ò¯Ð¹ â€” Telegram join request Ð·Ó©Ð²ÑˆÓ©Ó©Ñ€Ó©Ð³Ð´Ó©Ñ… Ò¯ÐµÐ´ Ð¼Ó©Ð½ ÑÐ½Ñ
        # chat_member event-Ð¸Ð¹Ð³ Ð°Ð²Ñ‚Ð¾Ð¼Ð°Ñ‚Ð°Ð°Ñ€ Ò¯Ò¯ÑÐ³ÑÐ´ÑÐ³ Ñ‚ÑƒÐ».
        pending = context.bot_data.get('pending_vip')
        ...
        msg = (
            ...
            f"(Ð–Ð¸ÑˆÑÑ: 3d = 3 Ñ…Ð¾Ð½Ð¾Ð³, 12h = 12 Ñ†Ð°Ð³, 30m = 30 Ð¼Ð¸Ð½ÑƒÑ‚, ÑÑÐ²ÑÐ» 1d12h)"
        )
...
async def cmd_add_vip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) != 3:
        await update.message.reply_text(
            "ðŸ“Œ Ð¥ÑÑ€ÑÐ³Ð»ÑÑ: /addvip [user_id] [chat_id] [Ñ…ÑƒÐ³Ð°Ñ†Ð°Ð°]\n"
            "Ð¥ÑƒÐ³Ð°Ñ†Ð°Ð°: 3d (Ñ…Ð¾Ð½Ð¾Ð³), 12h (Ñ†Ð°Ð³), 30m (Ð¼Ð¸Ð½ÑƒÑ‚), ÑÑÐ²ÑÐ» Ñ…Ð¾ÑÐ»ÑƒÑƒÐ»Ð¶ 1d12h"
        )
        return
    try:
        user_id = int(args[0])
        ...
        await update.message.reply_text("âŒ user_id/chat_id Ð±ÑƒÑ€ÑƒÑƒ Ñ„Ð¾Ñ€Ð¼Ð°Ñ‚.")
        return
    duration = db.parse_duration(args[2])
    if duration is None:
        await update.message.reply_text("âŒ Ð¥ÑƒÐ³Ð°Ñ†Ð°Ð°Ð½Ñ‹ Ñ„Ð¾Ñ€Ð¼Ð°Ñ‚ Ð±ÑƒÑ€ÑƒÑƒ. Ð–Ð¸ÑˆÑÑ: 3d, 12h, 30m, 1d12h")
        return
...
async def cmd_extend_vip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) != 3:
        await update.message.reply_text(
            "ðŸ“Œ Ð¥ÑÑ€ÑÐ³Ð»ÑÑ: /extendvip [user_id] [chat_id] [Ñ…ÑƒÐ³Ð°Ñ†Ð°Ð°]\n"
            "Ð¥ÑƒÐ³Ð°Ñ†Ð°Ð°: 3d (Ñ…Ð¾Ð½Ð¾Ð³), 12h (Ñ†Ð°Ð³), 30m (Ð¼Ð¸Ð½ÑƒÑ‚), ÑÑÐ²ÑÐ» Ñ…Ð¾ÑÐ»ÑƒÑƒÐ»Ð¶ 1d12h"
        )
        return
    try:
...
async def cmd_vip_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for v in vips:
        ...
        expiry = v['vip_expiry'][:16].replace('T', ' ') if v['vip_expiry'] else "â€”"
...
async def cmd_vip_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not memberships:
        ...
        for m in memberships:
            ...
            expiry = m['vip_expiry'][:16].replace('T', ' ') if m['vip_expiry'] else "â€”"
...
async def check_expiry_warnings(context: ContextTypes.DEFAULT_TYPE):
    """Ó¨Ð´Ó©Ñ€Ñ‚ 1 ÑƒÐ´Ð°Ð° (09:00) Ð°Ð¶Ð¸Ð»Ð»Ð°Ð¶, 3 Ð±Ð¾Ð»Ð¾Ð½ 2 Ñ…Ð¾Ð½Ð¾Ð³Ð¸Ð¹Ð½ Ð´Ð¾Ñ‚Ð¾Ñ€ Ð´ÑƒÑƒÑÐ°Ñ…
    Ñ…ÑÑ€ÑÐ³Ð»ÑÐ³Ñ‡Ð´ÑÐ´ ÑÐ°Ð½ÑƒÑƒÐ»Ð³Ð° Ð¸Ð»Ð³ÑÑÐ½Ñ."""
    bot = context.bot
...
async def check_expired_vips(context: ContextTypes.DEFAULT_TYPE):
    """Ð‘Ð°Ð¹Ð½Ð³Ð° (Ð¼Ð¸Ð½ÑƒÑ‚ Ñ‚ÑƒÑ‚Ð°Ð¼) Ð°Ð¶Ð¸Ð»Ð»Ð°Ð¶, Ñ…ÑƒÐ³Ð°Ñ†Ð°Ð° Ð´ÑƒÑƒÑÑÐ°Ð½ VIP-Ò¯Ò¯Ð´Ð¸Ð¹Ð³ ÑˆÑƒÑƒÐ´ Ñ…Ð°ÑÐ½Ð°."""
    bot = context.bot
